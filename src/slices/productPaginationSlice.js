...
export const productPaginationSlice = createSlice({
  ...
  reducers: {
    searchPagination: (state, action) => {
      state.search = action.payload.search; //objecto del componente de react
      state.pageIndex = 1;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getProductPagination.fulfilled, (state, { payload }) => {
        state.loading = false;
      ...
      .addCase(getProductPagination.rejected, (state, action) => {
        state.loading = false;
